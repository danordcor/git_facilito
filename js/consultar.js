...
$('#button_seleccion').click(function(){
	var criterio = $('#seleccion option:selected').val();
	var filtro = $('#filtro').val();
	//alert(filtro+" "+criterio);
	...
	if(filtro == "") {
        ...
        for(var i=0;i<regs;i++){
            ...
            if (datos.rows.length==0){
                ...
                tabla.push("</tbody><tfoot><tr><td colspan='5'> Listado de usuarios.</td></tr></tfoot></table>");
		...
		db.transaction(function(t){
		t.executeSql("SELECT * FROM usuarios WHERE ("+criterio+" LIKE ?)",["%"+filtro+"%"],function(tx, datos){
        var regs = datos.rows.length;
        for(var i=0;i<regs;i++){
                     tabla.push("<tr style='display: table-row;'><td>"+datos.rows.item(i).usuario+"</td><td>"+datos.rows.item(i).pass+"</td><td>"+datos.rows.item(i).descripcion+"</td></tr>");
        }
